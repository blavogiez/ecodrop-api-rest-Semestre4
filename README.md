...
|---|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `GET /users/leaderboard` | `WITH t(id, login, password, role, score, rank) AS (SELECT u.*, sum(dp.poids * wt.pointsPerKilo), rank() over(ORDER BY sum(dp.poids * wt.pointsPerKilo) DESC) FROM Users AS u JOIN Deposit AS dp ON u.id=dp.userId JOIN WasteType wt ON dp.wasteTypeId=wt.id GROUP BY u.id ORDER BY rank) SELECT id, login, password, role, score FROM t WHERE rank <= ?;` | CTE, Triple jointure, agrégation produit `poids × pointsPerKilo`, tri décroissant sur score calculé et récupération des dix premiers avec ex-aequo via un rank sur le score dans la CTE |
